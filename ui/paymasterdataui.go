...
// top selection menu
var InputAsHexItem = &Item[struct{}]{Label: "Input as HEX", Details: "Input directly as HEX"}
var EthInfVPMV7Item = &Item[struct{}]{Label: "EthInfinitism ValidatingPaymaster (V7)"}

// ValidatingPaymaster specific menu
var AfterItem = &Item[uint64]{Label: "Valid After", Details: "After this block time", Value: 0}
var UntilItem = &Item[uint64]{Label: "Valid Until", Details: "Until this block time", Value: 0}
var PaymasterSignatureItem = &Item[[]byte]{Label: "Paymaster Signature", Details: "Paymaster Signature", Value: []byte{}}

func SetPaymasterDataUI(it *Item[[]byte], usop *userop.UserOperation) error {
	prompt := promptui.Select{
		Label:     "Select an option",
		Items:     []MenuItem{InputAsHexItem, EthInfVPMV7Item, Back},
...
var PaymasterSignerItem = &Item[signer.Signer]{Label: "Paymaster Signer", Details: "Signer for Paymaster"}

func SetEthInfVPMV7DataUI(it *Item[[]byte], usop *userop.UserOperation) error {
	prompt := promptui.Select{
		Label:     "Set Validation Parameters",
		Items:     []MenuItem{ChainIDItem, AfterItem, UntilItem, PaymasterSignerItem, PaymasterSignatureItem, Set, Back},
		Templates: ItemTemplate,
		Size:      10,
	}
	...
	for {
		...
		case AfterItem.Label:
			InputUint64(AfterItem)
			after = AfterItem.Value

		case UntilItem.Label:
			InputUint64(UntilItem)
			until = UntilItem.Value
		...
		case Set.Label:
			...
			sig := PaymasterSignatureItem.Value
			if len(sig) > 0 {
				pmd = append(pmd, sig...)
			}
			if len(pmd) > 0 {
		...
		asigner := PaymasterSignerItem.Value
